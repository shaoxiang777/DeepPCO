...
from models.loss import MSELoss, MaskedMSELoss
...
def main(args=args, resume_train=False, save_epochwise=True, save_net=True):
    ...
    lr_scheduler = optim.lr_scheduler.StepLR(optimizer, 7, args["weight_decay"])
    ...
    train_set = EuRoC("train")
    test_set = EuRoC("test")

    torch.manual_seed(13)
    # train_data_len = int(0.9 * len(data_set))
    # test_data_len = int(0.1 * len(data_set))
    print(len(train_set))
    print(len(test_set))
